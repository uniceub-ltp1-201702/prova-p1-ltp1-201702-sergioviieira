...
public class BDSimulado {
    ...
    public BDSimulado(){
      ...
      //Criando Clientes
      Clientes c1 = new Clientes("Evandro da Silva", "987564123-89", LocalDate.of(1940, 9, 16), "1234-56789", "Rio de Janeiro", "RJ");
      Clientes c2 = new Clientes("João dos Montes", "457458459-45", LocalDate.of(2000, 1, 15), "1245-7896", "Brasilia", "DF");
      Clientes c3 = new Clientes("Maria do Socorro", "235678451-56", LocalDate.of(1988, 2, 26), "4758-6958", "Brasilia", "DF");
      Clientes c4 = new Clientes("Otaviano Neves", "895678451-91", LocalDate.of(1974, 4, 12), "2345-8945", "São Paulo", "SP");
      Clientes c5 = new Clientes("José das Couves", "111222333-44", LocalDate.of(1991, 9, 29), "2583-6985", "Brasilia", "DF");
      Clientes c6 = new Clientes("Regina Maria", "123456789-00", LocalDate.of(1950, 11, 6), "3265-8974", "Belo Horizonte", "MG");
   ...
   public String getInfoCliente(String nomeCliente){
	   String resultado = "";
//2a
	   for (int i = 0; i < this.clientes.size(); i++){
		   if (this.clientes.get(i).getNome().equals(nomeCliente)) {
			   resultado = "Nome: " + this.clientes.get(i).getNome()
					   + "/n" +
					       "CPF: " + this.clientes.get(i).getCPF()
					   + "/n" +
					       "Data: " + this.clientes.get(i).getData()
					   + "/n" +
					       "Telefone: " + this.clientes.get(i).getTelefone()
					   + "/n" + 
   ...
   public ArrayList<Integer> ligacoesPorCliente(String nome){
   ...
   public int ligacoesPorUF(String UF){
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getClientes().getUF().equals(UF)) {
				resultado++;
			}
		}
   ...
   public ArrayList<Integer> ligacoesUFDiferente(){
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getClientes().getUF() == this.ligacoes.get(i).getUFDest()) {
				resultado.add(this.ligacoes.get(i).getCodigo());
			}
		}
   ...
   public String contaCliente(String nome){
		String resultado = "";
		
	
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getClientes().getNome().equals(nome)) {
				resultado = "Nome: " + this.ligacoes.get(i).getClientes().getNome()
						+ "\n" +
						"CPF: " + this.ligacoes.get(i).getClientes().getCPF()
						+ "\n" +
					    "Data de Nascimento: " + this.ligacoes.get(i).getClientes().getData()
						+ "\n" +
					    "Telefone: " + this.ligacoes.get(i).getClientes().getTelefone()
					    + "\n" +
					    "Cidade: " + this.ligacoes.get(i).getClientes().getCidade()
					    + "\n" +
					    "UF: " + this.ligacoes.get(i).getClientes().getUF()
						+ "\n" +
					    "Ligações: " + this.ligacoes.get(i).getCodigo() + "\n" + this.ligacoes.get(i).getDataHoraI()
					    + "\n" + this.ligacoes.get(i).getTelDest() + "\n" + this.ligacoes.get(i).getUFDest();
			}
		}
